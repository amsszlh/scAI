...
for i = 1:n
    ...
    for j = 1:repeat
        if ~isempty(Inits)
            [W1,W2,H,Z,R] = scAI(X1,X2,K,alpha,lambda,gamma,s,stop_rule,seeds(i),'W1_INIT',Inits{i,j}.W1,'W2_INIT',Inits{i,j}.W2,'H_INIT',Inits{i,j}.H,'Z_INIT',Inits{i,j}.Z,'R',Inits{i,j}.R);
        else
            [W1,W2,H,Z,R] = scAI(X1,X2,K,alpha,lambda,gamma,s,stop_rule,seeds(i));
        end
